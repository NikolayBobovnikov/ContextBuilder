...
I have a script which provides UI for selecting source code files and making a makrdown file with all files put into the context. Please review correctness (although no immediate issues there, it works so far), and then rework it to produce asciidoc instead of markdown. Please insert the code into separate asciidoc codeblocks, with ---- guards and [source] tag.
